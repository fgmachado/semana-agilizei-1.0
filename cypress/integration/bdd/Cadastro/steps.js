...
Then(/^devo ser cadastrado com sucesso$/, () => {
    ...
    cy.wait('@postNewtable').then(res => expect(res.status).to.eq(200));
    cy.wait('@postUsertable').then(res => expect(res.status).to.eq(200));
    cy.wait('@getNewtable').then(res => expect(res.status).to.eq(200));
